                ...
                <div className="hidden sm:flex flex-col gap-2">
                  {gallery.map((src, i) => (
                    <button key={src} type="button" onClick={() => setActiveImage(i)}
                      className={`relative flex h-16 w-16 items-center justify-center overflow-hidden rounded-xl border-2 bg-white shadow-sm transition ${
                        i === activeImage ? "border-gray-900" : "border-transparent hover:border-gray-300"
                      }`}>
                      <Image src={src} alt={`${product.name} — vista ${i + 1}`} fill sizes="64px" className="object-cover" />
                    </button>
                  ))}
                </div>
              )}
